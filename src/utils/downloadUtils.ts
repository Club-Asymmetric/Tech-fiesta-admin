...
/**
 * Generate PDF blob for email attachment (similar to downloadRegistrationPDF but returns blob)
 */
export const generateRegistrationPDFBlob = (data: RegistrationDownloadData): Blob => {
  const doc = new jsPDF("p", "mm", "a4");
  const pageWidth = doc.internal.pageSize.getWidth();
  const pageHeight = doc.internal.pageSize.getHeight();
  const margin = 15;
